...
def get_dataset(dataset, data_path, batch_size=1, subset="imagenette", args=None):
    ...
    if dataset == 'CIFAR10':
        ...
        std = [0.3081]  # Standard deviation for MNIST
        if args.zca:
            transform = transforms.Compose([transforms.ToTensor()])
        else:
            transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])

        dst_train = datasets.MNIST(data_path, train=True, download=True, transform=transform)  # Training data
        dst_test = datasets.MNIST(data_path, train=False, download=True, transform=transform)  # Testing data
        class_names = [str(i) for i in range(num_classes)]  # Class names are just digits 0-9
        class_map = {x: x for x in range(num_classes)}
